...
void binary_tree_levelorder(const binary_tree_t *tree, void (*func)(int))
{
	...
	func(tree->n);
	childNode = get_children(childNode, tree);
	levels_rec(childNode, func);
...
void levels_rec(ll *head, void (*func)(int))
{
	...
	{
		func(curr->node->n);
		childNode = get_children(childNode, curr->node);
	}
	levels_rec(childNode, func);
	free_list(childNode);
}
...
ll *append(ll *head, const binary_tree_t *btnode)
{
	ll *newNode = NULL, *last = NULL;

	newNode = malloc(sizeof(*new));
	if (newNode)
	{
		newNode->node = btnode;
		newNode->next = NULL;
